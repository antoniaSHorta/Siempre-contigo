-- Migración para corregir la integración de alimentación con actividades
...
-- 2. Modificar el ENUM de tipo en la tabla actividad para incluir los nuevos tipos
ALTER TABLE actividad 
MODIFY COLUMN tipo ENUM('Medicamento', 'Terapia', 'Recreacional', 'Paseo', 'Ejercicio', 'Cita', 'Alimentacion', 'Videollamada');

-- 3. Modificar el ENUM de estado en la tabla actividad para ser consistente
ALTER TABLE actividad 
MODIFY COLUMN estado ENUM('Pendiente', 'En Progreso', 'Completado', 'Incompleto') DEFAULT 'Pendiente';

-- 4. Modificar el campo fecha para que sea DATETIME en lugar de DATE
ALTER TABLE actividad 
MODIFY COLUMN fecha DATETIME NOT NULL;

-- 5. Actualizar registros existentes si los hay (opcional)
